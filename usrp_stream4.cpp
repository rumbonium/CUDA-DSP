...
#define DIE(msg) { perror(msg); return; }
...
template<typename samp_type> void recv_to_file(
    ...
    meminfo smi(sizeof(samp_type), 4000000, 50);
    int shmid;
    key_t shm_key = 48879;
    samp_type *shm, *s;
    long unsigned int shm_pos = 0;
    if ((shmid = shmget(shm_key, smi.tot, IPC_CREAT | 0666)) == -1) DIE("shmget");
    if ((shm = (samp_type*)shmat(shmid, NULL, 0)) == (void*) -1) DIE("shmat");
    memset(shm,0,smi.tot);
    s = shm;
    memset(s, 0, 12*sizeof(float));
    ...
    for(i=0; i < smi.num; i++) { 
        if(semctl(sempty,i,SETVAL,semptyset) == -1) {printf("semctl %d\n",i); DIE("semctl");}
    }
    sfullset.val = 0;
    for(i=0; i < smi.num; i++) { 
        if(semctl(sfull,i,SETVAL,sfullset) == -1) {printf("semctl %d\n",i); DIE("semctl");}
    }    
    ...
    while(not stop_signal_called and (num_requested_samples != num_total_samps or num_requested_samples == 0)) {
        if(semctl(sempty,semid,GETVAL) == 1) {
            semptyset.val = 0;
            if(semctl(sempty,semid,SETVAL,semptyset) == -1) {printf("semctl %d\n",semid); DIE("semctl");}
            while(semid == old_semid) {
                ...
                copy(buff.begin(), buff.end(), s + shm_pos);
                //memcpy(s + shm_pos, buff, num_rx_samps);
                shm_pos = (shm_pos + num_rx_samps) % smi.ele;
                semid = shm_pos / smi.chk;
            ...
            sfullset.val = 1;
            if(semctl(sfull,old_semid,SETVAL,sfullset) == -1) {printf("semctl %d\n",old_semid); DIE("semsct");}
            old_semid = semid;
        }
    }
